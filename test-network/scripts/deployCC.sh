...
CC_SEQUENCE=${6:-"1"}
CC_INIT_FCN=${7:-"NA"}
CC_END_POLICY=${8:-"AND ('Org1MSP.member', 'Org2MSP.member') AND (OR 'Org3MSP.member', 'Org4MSP.member', 'Org5MSP.member', 'Org6MSP.member')"}
CC_COLL_CONFIG=${9:-"NA"}
DELAY=${10:-"3"}
MAX_RETRY=${11:-"5"}
VERBOSE=${12:-"false"}
...
## Install chaincode on all org's peers0
